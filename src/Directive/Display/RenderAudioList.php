...
class RenderAudioList extends \Baidu\Duer\Botsdk\Directive\BaseDirective{
    ...
    public function __construct($title, $behavior = self::REPLACE) {
        parent::__construct('Display.RenderAudioList');
        $data = array(
            'token' => $this->genToken(),
            'title' => $title,
            'behavior' => $behavior,
    ...
    public function setToken($token){
      ...
      */
    public function addAudioItem($audioItem){
        if($audioItem instanceof AudioItem){
            $this->audioItems[] = $audioItem;
            ++$this->data['size'];
        }
    }
    ...
    public function getData(){
        if($this->audioItems){
            foreach($this->audioItems as $audioItem){
                $this->data['audioItems'][] = $audioItem->getData();
            }
        }
        return $this->data;
    }
